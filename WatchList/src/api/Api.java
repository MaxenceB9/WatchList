...
public class Api {
    ...
    public CompletableFuture<Void> request(String query) {
        // la requette est asynchrone
        return CompletableFuture.runAsync(() -> {
            ...
            String url = String.format(
                "https://xmdbapi.com/api/v1/search?q=%s&limit=%d&apiKey=%s",
                encodedQuery,
                Settings.getInstance().getApiRateLimit(),
                Settings.getInstance().getApiKey()
            );
            ...
            try (HttpClient client = HttpClient.newHttpClient()) { // on crée un client http
                ...
                new Thread(() -> {
                    for (
                        int i = 0;
                        i < result.length();
                        i++ // on crée les media en fonction du res
                    ) {
                        ...
                        if (
                            fs.has("year") // on recup l'année
                        ) {
                            year = String.valueOf(fs.getInt("year"));
                        }
                        if (
                            fs.has("image") && !fs.isNull("image") // on récup l'image
                        ) {
                        ...
                        if (
                            fs.has("image") && !fs.isNull("image") // il faut une image pour que le media soit ajouter
                        ) {
    ...
    private List<String> getMoreData_Movie(String query) {
        ...
        String url = String.format(
            "https://xmdbapi.com/api/v1/movies/%s?apiKey=%s",
            encodedQuery,
            Settings.getInstance().getApiKey()
        );
        try (HttpClient client = HttpClient.newHttpClient()) { // on crée un client http
    ...
    private List<String> getMoreData_Serie(String query) {
        ...
        String url = String.format(
            "https://xmdbapi.com/api/v1/seasons/%s?apiKey=%s",
            encodedQuery,
            Settings.getInstance().getApiKey()
        );
        try (HttpClient client = HttpClient.newHttpClient()) { // on crée un client http
